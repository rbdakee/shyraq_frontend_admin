...
### A30 — Экран «Вход/выход» (QR-скан + ручная отметка): нет в готовом дизайне → новый роут `/attendance/checkin`, строим по дизайн-системе · resolved (2026-07-15, owner via AskUserQuestion)

**Контекст.** Backend выкатил attendance-register update (`docs/ATTENDANCE_REGISTER_FRONTEND_GUIDE.md`): админка теперь **ведёт** журнал — скан QR родителя (`POST /admin/qr/scan`), ручная отметка (`check-in`/`check-out`), структурные правки, удаление, история (HANDOFF §20). Готовый дизайн (`screens-ops.jsx` `AttendanceJournal`, L163) знает только два таба — «Журнал» (таблица + Edit-иконка) и «Дневной статус». Экрана сканирования там **нет**, как нет и кнопок удаления/структурной правки/истории. CLAUDE §6 запрещает молчаливое отклонение.

**Решение (owner, 2026-07-15).** Новый роут `/attendance/checkin` третьим пунктом подменю «Посещаемость», построенный **по существующей дизайн-системе** (те же токены/компоненты). Лейаут и все состояния — DESIGN §6.12.1. Владелец выбрал `/attendance/checkin` вместо киоск-фуллскрина (выпадал бы из шелла) и вместо таба внутри `/attendance` (камера монтировалась бы/размонтировалась при переключении табов, тесно рядом с таблицей).

**Движок сканера (owner).** Нативный `BarcodeDetector` + lazy-import `@zxing/browser` как fallback там, где API нет (Safari/Firefox). Ноль веса в типичном случае (Chrome/Edge/Android), работает везде. Отвергнуто: только zxing (~100kb всегда), только нативный (мёртвый экран в Safari/Firefox).

**Объём (owner, multi-select).** Входят: mobile-скан + ручная отметка на телефоне; редактируемый дневной статус (upsert); хинт «где ребёнок сейчас» + guard от дубля (§A33). **Не входит:** ручной ввод токена / поддержка USB-сканера-пистолета (скрытый input с автофокусом). **Известный риск от этого:** если оператор отказал в доступе к камере или её нет — запасного пути отметить по QR не остаётся, только ручная отметка по ФИО ребёнка. Revert path: input дешёвый, добавляется отдельным слайсом если владелец передумает.

**Trade-off.** Дизайн-дрейф в разделе «Посещаемость» (экрана и части действий нет в VIS). Revert path: переверстать в отдельном батче, контракты не затрагиваются.

**Связано:** §A31 (X-Device-Id — блокер), §A32 (роль reception), §A33 (хинт состояния), HANDOFF §20, PLAN §B27.

---

### A31 — `X-Device-Id` не подключён → блокер QR-скана; привязка создаётся при логине, перелогин обязателен · resolved (2026-07-15)

**Контекст.** `POST /admin/qr/scan` сверяет `X-Device-Id` с колонкой `device_id` активной сессии обычным SQL `=`. У клиента, залогинившегося без заголовка, там `NULL`, а `NULL = что_угодно` никогда не истинно ⇒ **любой** скан вернёт `401 no_active_session_for_device`, что бы фронт потом ни слал. Починить на стороне скана нельзя — привязка создаётся при логине.

**Ревизия кода (2026-07-15).** Плумбинг наполовину есть, но заголовок не уходит **никуда**: `api/auth.ts:75` `verifyOtp` принимает `deviceId`, но `routes/login.tsx:166` его не передаёт; `api/auth.ts:89` `selectRole` заголовок не шлёт вообще — а это **главная ловушка** гайда §1: `role/select` делает новый INSERT, а не ротацию, и мульти-садиковый админ (наш обычный кейс) теряет привязку даже если бы `otp/verify` был починен; `api/client.ts:30` `tryRefreshOnce` ходит голым `ky` мимо интерцептора.

**Решение.** `lib/device-id.ts` (UUID через `crypto.randomUUID()`, персист в `localStorage`, ленивая генерация) + заголовок **глобально** в `apiClient.beforeRequest` (лишним нигде не будет) + явно в `tryRefreshOnce` (мимо интерцептора) + в `verifyOtp`/`selectRole`. Первым слайсом батча — без него остальное физически не проверить.

**Последствие для QA.** Существующие сессии перепривязать нечем → после выката **все действующие пользователи должны перелогиниться**, иначе скан у них отвечает 401. Это не баг, это свойство схемы.

**Связано:** HANDOFF §20.1, PLAN §B27.

---

### A32 — Роль `reception` в Admin Web: ролевой гейт из гайда §5 не нужен · resolved (2026-07-15, backend-verified)

**Контекст.** Гайд §5 велит прятать «Перенести на другого ребёнка», «Поменять тип» и «Удалить» при `role === 'reception'`, и эндпоинты действительно помечены `@Roles('admin','reception')` (`admin-qr.controller.ts:108`). Возник вопрос — городить ли гейт.

**Проверка (backend, `auth.service.ts:54-58`).** `APP_ALLOWED_ROLES = { parent: ['parent'], staff: ['mentor','specialist','reception'], admin: ['admin'] }`, а `audienceForRole('reception')` → `'staff'`. Admin Web логинится с `app: 'admin'` (`api/auth.ts:6`) ⇒ **reception в админку войти физически не может** — он в Staff App. Роль в сессии Admin Web всегда `admin`.

**Решение.** Гейт **не реализуем** — это был бы мёртвый код. Все действия (удаление, структурная правка) показываем всегда. `@Roles('admin','reception')` на бэке существует потому, что те же роуты обслуживают Staff App; §5 гайда адресован ему, не нам. Записано, чтобы следующий человек не пошёл городить гейт заново по прочтении гайда.

**Watch.** Если бэк когда-нибудь пустит `reception` в аудиторию `admin` — гейт понадобится; проверять при изменениях `APP_ALLOWED_ROLES`.

**Связано:** HANDOFF §20.6, гайд §5.

---

### A33 — Хинт «где ребёнок сейчас» на карточке скана — трактовка §6 гайда «не считай статусы сами» · resolved (2026-07-15, owner-approved)

**Контекст.** Ответ `/admin/qr/scan` отдаёт родителя и его детей, но **не текущее состояние** каждого ребёнка — в саду он или уже ушёл. Оператор видит обе кнопки вслепую; «нажали не ту кнопку» — ровно тот кейс, ради которого в гайде §4.2 существует правка `eventType`. При этом §6 гайда прямо запрещает считать статусы на фронте.

**Решение (owner, 2026-07-15).** Хинт делаем. Лента событий за сегодня (`useAttendanceEvents({from:today,to:today})`) всё равно нужна для блока «Отмечено сегодня» — из неё строим `childId → последнее событие` и показываем на карточке «В саду с 08:42» / «Ушёл в 17:30» / «Ещё не приходил», подсвечивая вероятное действие. Плюс guard от дубля: повторный «Приход» при последнем `check_in` → подтверждение.

**Почему это не нарушает §6.** §6 запрещает **вычислять `child_intraday_status`** (демоушн `present → absent`, неприкосновенность `sick`/`on_vacation`) — там дублирование логики бэка гарантированно разъедется. Мы же показываем **факт из выдачи событий** («последнее событие — приход в 08:42») и **подсказываем** кнопку, не решая за оператора: обе кнопки остаются активными, настоящую проверку прав делает бэк на check-out. Граница тонкая — зафиксировано намеренно.

**Запрещено в рамках этого решения:** выводить из ленты `present/absent`, рисовать бейдж дневного статуса из событий, дизейблить кнопку по вычисленному состоянию.

**Связано:** HANDOFF §20.6, DESIGN §6.12.1, гайд §6.

---

### A34 — Row-actions в mobile-журнале посещаемости: секции журнала событий нет в готовом mobile-дизайне · resolved (2026-07-15, owner via AskUserQuestion)

**Контекст.** B27 добавил на desktop-журнале пять row-actions: «Корректировать», «Перенести на другого ребёнка», «Поменять тип», «Удалить», «История правок» (DESIGN §6.12.3). При портировании на mobile (слайс S7) выяснилось: готовый mobile-дизайн (`mobile-screens.jsx` L661–770) показывает только статистику/группы/сетку детей — **секции журнала событий там нет вообще**, а значит нет и места, откуда эти действия вызывать. CLAUDE §6 запрещает молчаливый дрейф от готового дизайна.

**Решение (owner, 2026-07-15).** Действия на мобиле **оставляем**, реализованы как bottom-sheet со списком действий по тапу на строку события. Отклонение зафиксировано здесь.

**Обоснование.** (1) §A30 уже признал дизайн-дрейф в разделе «Посещаемость» как таковой («экрана и части действий нет в VIS») — это расширение под тем же зонтиком, а не новое направление. (2) Секция событий на мобиле **уже существует** с B13/B19 — дизайн просто не описывал её действия. (3) Идиома `Sheet side="bottom"` + список действий — устоявшаяся в репо, не изобретена под этот случай. (4) Без этого мобильный админ не может ни поправить, ни удалить ошибочную запись — а «нажали не ту кнопку» это ровно тот кейс, ради которого правка `eventType` существует (гайд §4.2). Компоненты переиспользованы из desktop (`correction-modal.tsx`, `history-panel.tsx`), не продублированы.

**Trade-off.** Mobile-дрейф в разделе «Посещаемость» расширяется. Revert path: срезать action-sheet, действия останутся только на desktop; контракты не затрагиваются.
...
### A40 — Плеера камер нет в готовом дизайн-handoff · resolved (2026-09-14, решение владельца)

**Контекст.** В `docs/design/handoff/shyraq-admin/project/screens-ops.jsx` камеры показаны только как disabled-кнопка «Тест камеры» и Phase C-баннер. Готового дизайна плеера / экрана просмотра камер в handoff нет. Это отклонение от CLAUDE.md §6 (визуал 1:1 по готовому дизайну).

**Решение владельца (2026-09-14):**

1. **Минималистичная модалка** внутри существующей вкладки `/structure/cameras`, одна камера за раз. Нового экрана/роута не создавать.
2. **Сетки/мозаики камер не делать.** Причина: backend-гайд §3 прямо запрещает запрашивать 15 stream-токенов разом (TTL ~1 час, каждый открывает сессию на медиа-шлюзе).
3. **Ограничение H.265 принято.** Все камеры сейчас отдают H.265 — Chrome/Brave без аппаратного HEVC-декодера его не покажут, Safari покажет. Плеер обязан честно сообщать об этом (capability-гейт `MediaSource.isTypeSupported`), а не показывать чёрный квадрат.
4. Когда камеры переведут на H.264 — ничего не придётся переписывать: плеер выбирает транспорт по массиву `transports`, а не по `video_codec`.

**Обновление 2026-09-15 — камеры переведены на H.264, политика плеера изменена (решение владельца).**

Садик переключил sub-потоки камер с H.265 на H.264, цель — видеть трансляции в Chrome и на Android. Два следствия для фронта, оба уже в коде (B32-S4):

1. **Транспорт выбирается hls.js-first.** Нативный HLS используется только там, где браузер отвечает `probably` (Safari) или где hls.js физически не запустить (iPhone — нет `MediaSource`). Причина: Chrome на Android отвечает `maybe` на `canPlayType('application/vnd.apple.mpegurl')`, уходит на нативный путь и показывает чёрный кадр на live-fMP4 с токеном в query.
2. **Гейт по кодеку больше не блокирует воспроизведение.** `video_codec` в строке камеры — результат probe-джобы давностью до 30 минут; отказ строить плеер по нему прятал уже переключённую на H.264 камеру за предупреждением «откройте в Safari». Теперь плеер всегда пытается играть, а сообщение про H.265 показывается по фактической ошибке декодирования (`bufferAddCodecError` / `error` на `<video>`).

Пункт 3 выше (ограничение H.265 принято) остаётся в силе только как fallback для камер, которые не переключили или которым NVR вернул H.265.

**Не блокирует** — решение зафиксировано. Acceptance — IMPLEMENTATION_PLAN §B32. **Связано:** HANDOFF §9.2, DESIGN §6.6, C1 (камеры сняты с parked).
...
### C1 — Phase C: Face ID · parked

HANDOFF §22,§29: admin-эндпоинты конфигурации Face ID есть, но распознавание — edge, Phase C. **Строим UI как видимые заглушки** «доступно позже», к данным не подключаем. → B15 (Face табы).

**Камеры (CCTV):** снято с `parked` 2026-09-14. Backend CCTV задеплоен на dev, 15 камер первого садика отдают живое HLS-видео. Тест/просмотр камер больше не Phase C — реализуется в B32. `cameras/:id/test` как отдельный endpoint не существует и не планируется; его заменяет `GET /cameras/:id/stream` + `POST /cameras/:id/refresh-codec`.
