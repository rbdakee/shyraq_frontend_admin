...
- **Чтение медиа (обновлено 2026-06-21, backend media-update — см. OPEN_QUESTIONS §A26):** поля `media_urls[]` / `media_url` (лента, посты, истории, timeline, диагностика) теперь содержат **готовую абсолютную presigned-ссылку S3** вида `https://balam-media-dev.object.pscloud.io/<key>?X-Amz-Signature=…`. Вставляй её **напрямую** в `<img src>` / `<video src>` — `Authorization`-заголовок **НЕ** нужен (подпись встроена в URL), blob-fetch-костыль не нужен. ⏳ Ссылка живёт **1 час** — не сохранять надолго (стейт «навсегда», localStorage, БД); экран открыт >1ч → перезапросить данные (API вернёт свежие подписи). Фронт-митигация TTL — `refetchInterval` (50 мин) на медиа-несущих detail-запросах (`useContent`/`useChild`, §B26). Старый `GET /api/v1/media/{kgId}/{yyyyMm}/{filename}` (под JWT, kg-scoped) ещё работает как fallback, но не нужен. **Хост не хардкодим** — всегда берём URL из ответа.
- **Загрузка** (`POST /admin/content/upload-media`) — без изменений: возвращает **канонический неподписанный** `{url, key}` — их и сохраняем/прикрепляем к посту (не presigned-ссылку).
...
| GET    | `/admin/schedule/templates/:id`               | Деталь: `ScheduleTemplateResponseDto` с **встроенным** `slots: ScheduleTemplateSlotResponseDto[]` = `{id, dayOfWeek, startTime, endTime, activityName, category, locationId?, description?}`. `category` = `SlotCategory` enum (`'lesson'\|'activity'\|'meal'\|'sleep'`), всегда присутствует (B-category, backend a1522b0). **Нет** отдельного `GET …/:id/slots` (бэкенд не реализует) — слоты читаем из этого ответа и из списка (§GET templates). |
| PATCH  | `/admin/schedule/templates/:id`               | `UpdateScheduleTemplateDto = {name?, isActive?, validUntil?}`.                                                                                                                                                                                                                                                                                                                                                                                       |
| POST   | `/admin/schedule/templates/:id/slots`         | `CreateSlotDto = {dayOfWeek*(mon..sun), startTime*, endTime*, activityName*, category?(default 'activity'), locationId?, description?}`. **Ответ (201): весь обновлённый `ScheduleTemplateResponseDto`** (с полным `slots[]`), НЕ одиночный слот.                                                                                                                                                                                                    |
| PATCH  | `/admin/schedule/templates/:id/slots/:slotId` | `UpdateSlotDto = {dayOfWeek?, startTime?, endTime?, activityName?, category?, locationId?(nullable), description?(nullable)}`. **Ответ (200): весь `ScheduleTemplateResponseDto`**, НЕ одиночный слот.                                                                                                                                                                                                                                               |
| DELETE | `/admin/schedule/templates/:id/slots/:slotId` | Удалить слот. **Ответ 204 (без тела).**                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| POST   | `/admin/schedule/activity-events`             | `CreateActivityEventDto = {groupId*, activityName*, category?(default 'activity'), locationId?, startsAt*, endsAt?, notes?}` — ad-hoc событие. См. §B2.                                                                                                                                                                                                                                                                                              |
| PATCH  | `/admin/schedule/activity-events/:id`         | `UpdateActivityEventDto = {activityName?, category?, locationId?, startsAt?, endsAt?, notes?}`.                                                                                                                                                                                                                                                                                                                                                      |
| DELETE | `/admin/schedule/activity-events/:id`         | Удалить событие.                                                                                                                                                                                                                                                                                                                                                                                                                                     |

`ActivityEventResponseDto` = `{id, kindergartenId, groupId, templateSlotId?, activityName, category('lesson'\|'activity'\|'meal'\|'sleep'), locationId?, startsAt, endsAt?, status('scheduled'\|'in_progress'\|'completed'\|'cancelled'), createdBy?, notes?, createdAt, updatedAt}`. `category` копируется из слота при проекции; ad-hoc событие → default `'activity'` (B-category, backend a1522b0).
...
> - **`provider` enum (подтверждено live 2026-07-24, prod-инцидент — `/billing/payments` падал в "Не удалось загрузить данные"):** `mock | halyk_epay | kaspi_pay | tiptoppay | freedom_pay | bcc | cash`. Провайдер `bcc` (Bank CenterCredit) добавлен backend'ом без апдейта наших docs; фронтовый Zod-enum в `api/payments.ts` его не знал → `PaymentListResponseSchema.parse` кидал на первой же `bcc`-записи → весь список падал целиком (client-side, не backend outage). Пофикшено: `bcc` добавлен в `PaymentProviderEnum`, `PROVIDER_I18N_KEYS`, фильтр провайдеров на `/billing/payments`, `locales/{ru,kk}/billing.json#payments.provider.bcc`.
...
| Расписание, меню, контент, Qundylyq                 | ✅ готово               | Медиа — **presigned S3** (абсолютный URL в `media_url(s)`, TTL 1ч, без auth-заголовка; §2.6/§A26). Старый `/api/v1/media/*` под JWT — fallback. Хост из ответа. |
