...
- [x] Журнал фильтруется (ребёнок/метод/диапазон), корректировка сохраняется (pickup*user_id только на check_out); daily-status доска + сводка отсутствий из `attendance-today` (per §A23). *(B13 wave 2026-06-04: attendance/daily-status DTO без child name/group → client-side join через `useChildrenList`; bare-array offset без `total` → load-more по полноте страницы. Mobile-stub'ы переподключены к реальным хукам.)\_
- [x] Конструктор шаблона строит schema (секции→поля 7 типов, options/min-max условно, key-uniqueness); при заполненных записях schema-редактирование заблокировано (409 `template_has_entries` баннер, name/description/deactivate остаются). Gate exit 0 (366 tests). _(Live-расхождение §20 `/daily-status/summary` → reuse attendance-today, зафиксировано HANDOFF §20 + OPEN_QUESTIONS §A23. Dual-review: reviewer-opus поймал cursor-пагинацию (useInfiniteQuery) + useMemo→useEffect, исправлено.)_
